/* ============================================================================
   ...
   below for exactly one situation: the server being unreachable, so the site
   still demos offline.

   That result carries isMock:true and the interface says so. A mocked result
   can never be silently mistaken for a real verdict.
...
(function (global) {
  ...
  async function analyzeVideo(file, options) {
    ...
    /* Anything with no real bytes behind it cannot be uploaded, so it stays
       mocked. Every file the UI can now produce is a real Blob. */
    ...
    /* ----- mocked path (server unreachable) ----- */
    var percent = 0;
    while (percent < 100) {
    ...
    var mocked = buildResult(file, Date.now() - startedAt);
    mocked.isMock = true;
    return mocked;
  }
