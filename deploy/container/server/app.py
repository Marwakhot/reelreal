...
# --------------------------------------------------------------------------
# The website, served from this same app
# --------------------------------------------------------------------------
# On a Space there is one public URL, so the site is served from "/" and the
# API answers on /health and /v1/analyze beneath it. Because the page and the
# API then share an origin, the browser needs no CORS permission for the
# upload and config.js needs no hard-coded server address.
#
# This mount is added last on purpose. Starlette matches routes in the order
# they were registered, so the two API routes above win and only unmatched
# paths reach the static files. html=True makes "/" serve index.html.
